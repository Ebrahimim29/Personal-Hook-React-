    ...
    return (
        <div>
            ...
            <div className="grid gap-6 grid-cols-1 md:grid-cols-2">
                {data.map((post) => (
                    <div key={post.id} className="bg-white/30 border border-gray-200 rounded shadow-lg overflow-hidden hover:bg-blue-200
                    transition-shadow duration-300">
                        <div className="bg-linear-to-r from-orange-500 to-pink-300 h-3"></div>
                        <div className="p-5">
                            <h3 className="font-bold text-lg text-gray-800 mb-3 line-clamp-2">{post.title}</h3>
                            <p className="text-gray-600 text-sm mb-4 line-clamp-3">{post.body}</p>
                            <div className="flex justify-between items-center">
                                <span className="bg-orange-200 text-orange-800 text-xs px-2 py-1 rounded">پست #{post.id}</span>
                                <button className="text-sm text-white/100 bg-linear-to-r from-orange-600 to-blue-300 px-3 py-1
                                rounded hover:scale-150 transition-transform duration-300">
                                    ادامه مطلب
                                </button>
                            </div>
                        </div>
